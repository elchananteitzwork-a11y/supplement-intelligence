...
Amendment Log:
  ...
  - "v1.11 (2026-07-17): M2.20 shipped — the sixth Evidence Depth Cluster milestone (see docs/PRODUCT_INTELLIGENCE_V2_ROADMAP.md for the full completion write-up). Added a deterministic firm-scoped FDA recall-history lookup (lib/regulatory-engine/manufacturer-credibility.ts, reusing index.ts's existing food/enforcement.json Class I/II/III counting logic) applied to real competitor manufacturer/brand identity from Keepa/Apify, surfaced as a new, purely additive field on top_competitors[]. Narrowed at the R&D stage from the milestone's headline 'manufacturing credibility' to recall history only, confirmed directly with the owner: live research confirmed no structured source exists for FDA facility registration or GMP/inspection data for food/supplement facilities (only a medical-device registration endpoint exists), and FDA's real inspection/warning-letter datasets are bot-protected bulk files, not a keyless API. Zero changes to scoring, verdict logic, kill switches, AI interpretation, or UI. Live-validated against real openFDA data including a confirmed real recall record and a confirmed whitespace-trim bug fix, zero regression. Adopted the owner's permanent Engineering Workflow & Token Efficiency Policy for this and all future milestones (incremental context loading, risk-tiered review depth, autonomous continuation absent a genuine owner decision)."
  - "v1.12 (2026-07-18): M2.21 shipped — the seventh and final Evidence Depth Cluster milestone (see docs/PRODUCT_INTELLIGENCE_V2_ROADMAP.md for the full completion write-up). Added a new, standalone, additive Evidence Depth Score (lib/evidence-depth-score/) synthesizing all six prior cluster signals into one coverage-honest composite on MemoData, never wired into scoring.ts/verdict-matrix.ts/GroundedScore (a full Decision Engine synthesis was considered and explicitly rejected at the R&D stage — would require a SCORING_ENGINE_VERSION bump per Constitution Law 20 and inherits a real sparse-coverage problem from the fixed 3-ingredient tracked list). Cluster-wide invariant confirmed across all 7 milestones: no Evidence Depth Cluster field has ever been wired into the real Decision Engine. Before this milestone, the owner separately requested and received real architecture/rate-limit research into replacing the fixed TRACKED_INGREDIENTS list with dynamic coverage; chose to ship M2.21 on current coverage and deferred that as its own future milestone (research preserved, not repeated). This completes the Evidence Depth Cluster (M2.15-M2.21) in full."
  - "v1.13 (2026-07-18): M2.22 shipped — Divergence Detector (see docs/PRODUCT_INTELLIGENCE_V2_ROADMAP.md for the full completion write-up). Confirmed via architecture research to be a low-ambiguity, mechanical extension of already-shipped M2.11/M2.12 infrastructure, not a fork — extracted the M2.12 Velocity Detector's acceleration primitive into a shared lib/pattern-detection/ module (zero behavior change to the existing Discovery Engine, re-verified against its own unchanged test suite) and built a sibling Divergence Detector reusing the same niche_timeseries read path, writing to a new append-only divergence_alerts table. Never wired into scoring/verdict logic, same posture as Discovery Alerts. Independent review caught and the implementer fixed a real duplicate-fetch efficiency issue between the two detectors sharing one cron run. Live logic exercise against real production niche_timeseries data produced one real, correctly-thresholded divergence result. Production validation is partial and honestly disclosed: the new migration has not yet been applied and this code was not yet deployed at validation time — full live-cron confirmation is a pending follow-up, not silently marked complete."
  - "v1.14 (2026-07-18): M3.5 shipped — TikTok Shop Intelligence / Social Commerce (see docs/PRODUCT_INTELLIGENCE_V2_ROADMAP.md for the full completion write-up). Live research found neither Kalodata nor FastMoss offers self-serve API access (both Enterprise-sales-gated; Kalodata's public ToS bars commercial/automated use); a second pass found a real, self-serve alternative via Apify's pratikdani/tiktok-shop-search-scraper actor, matching the existing junglee/amazon-crawler integration pattern. Shipped as a fully isolated 'social_commerce' signal (Path B, chosen over blending into the existing demand composite, per explicit engineering recommendation and owner decision) — structurally excluded from engine.ts's cross-provider aggregation, not merely omitted from BASE_WEIGHTS, and deliberately not registered in the shared provider registry after a real cost bug (every live analysis would have paid for a discarded Apify call) was caught and fixed mid-implementation. Persists repeated niche_timeseries snapshots via the existing watchlist-recheck pipeline, laying groundwork for the already-built M2.12/M2.22 pattern-detection primitives to later derive bounded-period velocity. Ships with a documented Social Commerce Calibration Gate (real exported numeric constants) specifying exactly what must be true — including that it may never independently trigger a BUILD_NOW/AVOID verdict while LIMITED confidence — before any future milestone may wire it into scoring. Independent review caught and the implementer fixed a real honesty-critical bug: a MIN_RESULTS gate checking raw item count instead of usable count, which could have persisted fabricated zero-GMV readings into the calibration history. Production validation is partial and honestly disclosed: the Apify account is confirmed still over its real usage cap (live 403), so the live success-path call could not be exercised — the real failure-path handling is unit-tested."
---
