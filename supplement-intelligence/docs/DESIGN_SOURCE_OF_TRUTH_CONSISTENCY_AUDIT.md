...
4. **Exact fix — add to "The One World principle," after the inheritance list:**
   > **One world ≠ one photograph.** Per an explicit owner correction (2026-07-21, recorded in `AmbientWorld.tsx`'s doc comment): every major screen gets **its own** composition and atmosphere — same visual grammar (palette, grade, scrim, glass, particles), different image. `landing-cathedral-of-palms.jpg` belongs to Landing/Login; `candidate-detail-cathedral-of-palms-night.jpg` belongs to Candidate Detail. **Dashboard and Discover have no approved composition yet.** Producing one is a required, owner-gated step of each phase: generate candidates in the same world → owner picks → only then implement. Reusing another screen's photo verbatim is a defect, not a shortcut — `AmbientWorld`'s required `image` prop exists to force this choice.
