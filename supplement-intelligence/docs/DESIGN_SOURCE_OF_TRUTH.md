...
| **Candidate Detail** | Real content/data: `app/memo/[id]/page.tsx` + `components/memo/MemoDisplay.tsx` (14 real sections) + `components/pi/candidate-core/` (real WebGL hero, see below). The dark "night world" prototype (`design-prototypes/candidate-detail-night.html` + the night photo) is **superseded by the 2026-07-22 cream directive — archived exploration, do not implement.** New direction: cream-register recreation per `DESIGN_LANGUAGE_PLAN.md` §5 (WebGL rotor on a bright cream stage), pending its own mockup → owner approval. | Direction changed 2026-07-22; cream mockup not yet started. |
...
| Candidate Detail | `app/memo/[id]/page.tsx` | Layer 2 body (`MemoDisplay.tsx`, 14 real sections) + Layer 2 hero (`CandidateCoreHero`, real WebGL 3D rotor — see §3) | ~~night prototype~~ superseded 2026-07-22 (archived) | **Cream-register recreation pending** — `DESIGN_LANGUAGE_PLAN.md` §5, mockup gate applies. |
