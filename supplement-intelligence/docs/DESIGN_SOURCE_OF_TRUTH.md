...
| **Background compositions** | Two locked, in production use (day + night Cathedral of Palms, listed above). **The day image is the foundation for Landing, Login, AND Home** (owner correction, 2026-07-22 — Home does not get a distinct photograph; distinctiveness comes from treatment — crop/`imagePosition`/intensity/scrim — not a new image). Everything else generated during exploration (round 1: lagoon/rainforest/solar-horizon; round 2: the two runner-up candidates alongside the chosen Cathedral of Palms; a golden-dusk Home round) lives only in the scratchpad and **was explicitly not chosen** — flagged so it's never mistaken for approved. **No new environment composition for any screen without an explicit owner-approved replacement.** | Two locked compositions, intentionally reused across screens via treatment; rest is rejected exploration history. |
