...
1. **Cinematic register** — an immersive hero moment. Full-bleed photo or video, dark color-grade scrim, `AmbientParticles`, glass panels (`GlassPanel`/`GlassInstrument`/`ProofCard`), cine motion tokens. **Currently: Landing only — and per the 2026-07-22 "recreate everything on cream" directive (`docs/DESIGN_LANGUAGE_PLAN.md`), it stays Landing-only. Candidate Detail's dark prototype is superseded (archived, not implemented).**
...
- **Resolved later the same day (2026-07-22):** the owner directed a full recreation of all remaining screens on the cream register, harmonized with the Landing ("use the basic of the design system colors we have now and make sure it also fits the color of the landing page"). The full language definition, component kit, and rollout order live in **`docs/DESIGN_LANGUAGE_PLAN.md`** — cream is now the confirmed register for every screen except Landing, including Candidate Detail (whose dark night prototype is superseded and archived, flagged for explicit owner confirmation at its cream mockup).
...
| **Dashboard / Home** | **SHIPPED 2026-07-22** (UIv2-M3, `docs/rd/RD-UIv2-M3-home-rebuild.md`, commit `6a3aaa5`): cream register, not the cinematic/photo register originally planned here — see the "Warm Intelligence" direction update above, which superseded a cinematic Home before this milestone started. `app/dashboard/page.tsx` merges the former `/dashboard` (quota, aggregates, `computeCardIntelligence`) and `/pipeline` (stage-grouped rows, anchor sentence, real attention events, frozen vocabulary) into one screen. New `components/pi/HomeShell.tsx` top-nav scoped to this page only. `/pipeline` deleted (route + `PipelineView.tsx`/`WhatChangedStrip.tsx`, confirmed orphaned). Full gate run: mockup → owner approval → R&D → implement → simplify → independent review (caught + fixed a real `avgScore` consistency bug) → QA (tsc/vitest/build clean, live verification, all PASS). | **Implemented and shipped.** |
...
| **Candidate Detail** | **SHIPPED 2026-07-22** (UIv2-M4, `docs/rd/RD-UIv2-M4-candidate-detail-cream.md`, commit `49f0de1`). `app/memo/[id]/page.tsx` + `components/memo/MemoDisplay.tsx` (14 real sections, unchanged, now gated behind a `Sources · N →` toggle — `app/memo/[id]/MemoDetailBody.tsx`) + `components/pi/candidate-core/` (real WebGL hero, cream stage, real Environment/Lightformer reflection + contact shadow, both reduced-motion-verified). The dark "night world" prototype (`design-prototypes/candidate-detail-night.html`) stays archived, never implemented. Full gate run: mockup → 5-second-comprehension revision → real material R&D (motion-3d-agent, owner reviewed live) → R&D doc → implement → simplify → independent review → QA (all PASS). | **Implemented and shipped.** |
...
| Dashboard / Home | Canonical URL: `/dashboard`. **SHIPPED 2026-07-22**, UIv2-M3 (cream register, not cinematic — `docs/rd/RD-UIv2-M3-home-rebuild.md`) | Cream `pi-*`, merged IA from both former routes | — | **Done.** Next in the rollout order: Candidate Detail. |
...
| Candidate Detail | `app/memo/[id]/page.tsx` | **SHIPPED 2026-07-22**, UIv2-M4 — cream register, real WebGL hero (`CandidateCoreHero`/`CandidateCoreCanvas`) with real reflection+shadow, 14-section body gated behind Sources | ~~night prototype~~ superseded, archived, never implemented | **Done.** Next in the rollout order: Discover. |
...
| `/pipeline` | ~~`app/pipeline/page.tsx`~~ **deleted 2026-07-22** (UIv2-M3) | — | — | **Gone.** Merged into `/dashboard`; both `/pipeline` and `/pipeline/preview` 404 live, confirmed by QA. |
