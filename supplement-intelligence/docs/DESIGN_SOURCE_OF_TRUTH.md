...
**The Cathedral of Palms is not a Landing background. It is the visual identity of the entire product.** Every screen inherits the same:
...
|---|---|---|
| **Cathedral of Palms Hero** | `public/ambient/landing-cathedral-of-palms.jpg` (day — locked, owner's exact words: "keep Option A exactly as it is") + `public/ambient/candidate-detail-cathedral-of-palms-night.jpg` (night companion, generated this session, real moonlight/no fantasy effects, approved as of the last polish-pass review) | **Locked identity, foundation for the whole product per today's instruction.** |
...
| **Dashboard** | **Two real, parallel implementations found — not one.** (1) `app/dashboard/page.tsx` (161 lines) — currently linked in `SideNav` as "Home," stat-tile/card-grid layout (`DashboardOpportunityCard`, `StatTile`), plain "Home" headline, does **not** use the frozen product vocabulary. (2) `app/pipeline/page.tsx` + `components/pi/PipelineView.tsx`/`StageGroup.tsx`/`CandidateRow.tsx`/`WhatChangedStrip.tsx` (~304 lines) — a real, fully compiling, functional route that is **not linked anywhere in navigation**. Stage-grouped (Shortlisted/Analyzed/Hunches/Committed·Killed), a single serif anchor sentence, a real "What Changed" strip, honest ghost-stages for unbuilt features — and it **does** use the frozen "Hunch" vocabulary ("Log a hunch" CTA). Built per `docs/rd/RD-UIv2-M1-pipeline-home.md` (2026-07-20) explicitly as "the first screen of the approved UX Architecture v1 build order," intended for a "staged cutover" to become the real Home — that cutover apparently never happened; `/dashboard` was independently re-skinned onto pi-\* the next day (`7db3583`) instead. The scratchpad mockup `dashboard_v1.final.html` / Artifact "Product Intelligence — Home" matches route (1), not (2). Cinematic version of either: does not exist yet. | **Genuine open question, not resolved here — see §6 item 6.** Phase 4 cannot start until this is decided; picking one silently would risk building on top of the wrong IA. |
...
| **Background compositions** | Two locked, in production use (day + night Cathedral of Palms, listed above). Everything else generated during exploration (round 1: lagoon/rainforest/solar-horizon; round 2: the two runner-up candidates alongside the chosen Cathedral of Palms) lives only in the scratchpad and **was explicitly not chosen** — flagged so it's never mistaken for approved. | Two locked compositions; rest is rejected exploration history. |
...
|---|---|---|---|---|
| Landing | `app/page.tsx` | **Layer 3, implemented** | — | Done. Cathedral of Palms hero, locked. |
| Login | `app/login/page.tsx` | **Layer 3, implemented** | — | Done. Same hero, single glass instrument, no split-screen. |
| Dashboard | `app/dashboard/page.tsx` **AND** `app/pipeline/page.tsx` (two real, unreconciled routes — see §0) | Layer 2 (pi-\*) — `/dashboard` linked in nav, `/pipeline` orphaned but functional | `scratchpad/dashboard_v1.final.html` matches `/dashboard` only | **Blocked, not "not started."** Phase 4 cannot proceed until §6 item 6 (which route is the real IA) is resolved. |
...
| Pipeline / Watchlist / Alerts / Leaderboard / Settings / History | various | Layer 2 (pi-\*) | — | **Explicitly out of scope.** Stays on `AppShell`. |
...
6. **Which route is the real Dashboard/Home — found during a Dashboard-specific audit (2026-07-22), blocks Phase 4.** `/dashboard` (linked in nav, stat/card-grid, plain "Home" headline) and `/pipeline` (not linked in nav, fully functional, stage-grouped, uses the frozen "Hunch" vocabulary, built explicitly per its own R&D doc as "the first screen of the approved UX Architecture v1 build order" with an intended-but-never-executed "staged cutover" to become Home) are **both real**. They were never reconciled — `/dashboard` was independently re-skinned onto pi-\* the day after `/pipeline` was built, instead of the planned cutover happening. This is not a case of "pick the prettier one" — `/pipeline` is the one that actually matches the locked product language, `/dashboard` is the one actually in front of users today. Needs an explicit answer before any cinematic Dashboard work starts: cut over to `/pipeline`'s IA, keep `/dashboard`'s, or merge.
