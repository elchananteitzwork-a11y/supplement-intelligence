...
export default function MemoPage() {
  ...
  return (
    <main className="min-h-screen bg-pi-cream font-sans text-pi-ink max-w-3xl mx-auto px-6 py-12 space-y-8">
      {/* Breadcrumb */}
      <div className="flex items-center gap-3 text-xs font-mono text-pi-faint uppercase flex-wrap">
        ...
        <span className="text-pi-faint">/</span>
        <Link href={`/research/${signal_id}/opportunity`} className="hover:text-pi-ink transition-colors">Opportunity</Link>
        <span className="text-pi-faint">/</span>
        <Link href={`/research/${signal_id}/evaluate`} className="hover:text-pi-ink transition-colors">Evaluate</Link>
        <span className="text-pi-faint">/</span>
        <span className="text-pi-sub">Investment Memo</span>
      </div>

      <div className="space-y-1 border-b border-pi-hairline pb-4">
        <h1 className="font-serif text-[26px] font-semibold leading-snug tracking-tight text-pi-ink">Investment Memo</h1>
      ...
      {theses.length > 1 && (
        <div className="space-y-2">
          <p className="text-[11px] font-mono font-semibold text-pi-faint uppercase tracking-wider">Thesis</p>
            ...
            {theses.map((t, i) => {
              ...
              return (
                <button
                  ...
                  className={`text-left border px-3 py-2.5 transition-colors bg-pi-card ${
                    selected === t.id
                ...
                >
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className="text-xs font-mono text-pi-faint">#{i + 1}</span>
                    {ready ? (
                      <span className="text-[10px] px-1.5 py-0.5 border border-pi-build/40 text-pi-build font-mono uppercase">
                        Stage 3 done
                      </span>
                    ) : debateReady.size > 0 ? (
                      <span className="text-[10px] px-1.5 py-0.5 border border-pi-hairline text-pi-faint font-mono uppercase">
                        needs Stage 3
                      </span>
                    ) : null}
                  </div>
                  <p className={`text-xs mt-1 ${selected === t.id ? 'text-pi-ink' : 'text-pi-sub'}`}>
                    {t.product_angle}
                  </p>
                </button>
              )
      ...
      {selectedNotReady && (
        <div className="rounded-xl border border-pi-gold/40 bg-pi-card px-4 py-3 text-xs text-pi-gold space-y-1">
          ...
          <Link
            ...
            className="inline-block mt-1 underline hover:text-pi-ink"
          >
            Go to Stage 3 →
          </Link>
        </div>
      ...
      <HardCard variant="pi" className="space-y-3">
        <button
          ...
          className="text-xs font-mono uppercase tracking-wide text-pi-sub hover:text-pi-ink flex items-center gap-2"
        >
        ...
        {showInputs && (
          <div className="grid grid-cols-3 gap-3 pt-2">
            <div className="space-y-1">
              <label className="text-xs text-pi-sub">Actual COGS/unit ($)</label>
              <input
                type="number" value={actualCOGS}
                onChange={e => setActualCOGS(e.target.value)}
                placeholder="e.g. 6.50"
                className="rounded-xl w-full border border-pi-hairline bg-pi-card px-3 py-1.5 text-xs text-pi-ink focus:outline-none focus:ring-1 focus:ring-pi-gold-bright"
              />
            </div>
            <div className="space-y-1">
              <label className="text-xs text-pi-sub">Target launch price ($)</label>
              <input
            ...
            <div className="space-y-1">
              <label className="text-xs text-pi-sub">Monthly ad budget ($)</label>
              <input
      ...
      {stage === 'generating' && (
        <p className="text-xs text-center text-pi-faint font-mono animate-pulse">
      ...
      {memo && stage === 'done' && (
        <div className="space-y-6">
          {fromCache && (
            <div className="flex items-center justify-between text-xs font-mono text-pi-faint">
              <span>From cache.</span>
              <button
                onClick={() => { setMemo(null); setStage('idle'); setFromCache(false) }}
                className="underline hover:text-pi-ink"
              >
                Regenerate
              </button>
            </div>
          ...
          {/* Completion CTA */}
          <div className="border-t border-pi-hairline pt-8 flex flex-col sm:flex-row items-start sm:items-center gap-4">
            <PrimaryLinkButton variant="pi" href="/research">Analyze another product →</PrimaryLinkButton>
            <GhostLinkButton variant="pi" href="/research">Back to research</GhostLinkButton>
          </div>
        </div>
      )}
    </main>
