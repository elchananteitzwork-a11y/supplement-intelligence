    ...
    <div className="min-h-screen bg-pi-cream pb-16 text-pi-ink">
      ...
      <div className="mx-auto max-w-[640px] px-5 pt-10">
        {chapter.key === 'gap' && gap ? (
          ...
          <>
            <p className="mb-6 max-w-[65ch] break-words text-[15px] leading-relaxed text-pi-sub">{chapter.headline}</p>

            {chapter.rows.length > 0 && (
              <RecordCard className="mb-6 divide-y divide-pi-hairline px-5">
                {chapter.rows.map((row, i) => (
                  <RecordCardRow key={i} claim={row.claim} value={row.value} marker={row.marker} />
                ))}
              </RecordCard>
            )}
            ...
            {chapter.read && (
              <RecordCard className="relative rounded-xl py-4 pl-5 pr-4 shadow-[0_1px_2px_rgba(22,23,26,0.04)]">
                ...
                <p className="max-w-[65ch] break-words font-serif text-[16px] italic leading-relaxed text-pi-ink">
