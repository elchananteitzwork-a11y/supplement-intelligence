...
function IndependenceConfidenceBlock({ assessment }: { assessment: ConfidenceAssessment }) {
  ...
  return (
    <div className="mt-7 pt-6 border-t border-pi-noir-hairline">
      <div className="flex items-baseline justify-between gap-3 mb-2">
        <p className="text-[10px] text-pi-noir-sub uppercase tracking-widest">Independence-Aware Confidence</p>
        <span className={`font-mono text-lg font-bold ${pct !== null && pct >= 50 ? 'text-pi-build-noir' : pct !== null && pct >= 25 ? 'text-pi-gold-bright' : 'text-pi-noir-sub'}`}>
          {pct !== null ? `${pct}%` : '—'}
        </span>
      </div>
      <p className="text-[11px] text-pi-noir-sub mb-1">
      ...
      <div className="mt-3 space-y-2.5">
        {assessment.dimensions.map(d => (
          <div key={d.key} className="flex items-center gap-3">
            <span className="text-xs text-pi-noir-sub w-40 shrink-0 truncate">{d.label}</span>
            {d.confidence !== null ? (
              <>
                <WitnessDots filled={d.confirmingChannelCount} total={Math.max(d.witnesses.length, d.confirmingChannelCount, 1)} size="sm" variant="pi-noir" />
                <span className="font-mono text-xs text-pi-noir-sub w-10 text-right shrink-0">{Math.round(d.confidence * 100)}%</span>
              </>
            ) : (
              <span className="flex-1 text-xs text-pi-noir-sub italic">No real evidence to be confident about</span>
            )}
  ...
  return (
    <PiCard padded={false} className="rounded-2xl p-6 sm:p-8 animate-in">
      <div className="flex items-center justify-between gap-3 mb-6">
        <p className="text-[11px] font-mono font-semibold uppercase tracking-[0.18em] text-pi-noir-text">Evidence &amp; Confidence</p>
        <ConfidencePill level={confidence.level} note={confidence.note} />
      </div>

      {facts.length > 0 && (
        <div className="flex gap-6 mb-6 pb-6 border-b border-pi-noir-hairline sm:hidden">
          {facts.map(([l, v]) => (
            <div key={l} className="text-center">
              <p className="text-[10px] text-pi-noir-sub uppercase tracking-wider">{l}</p>
      ...
      {categoryCreationContext && (
        <div className="mb-5 rounded-lg border border-pi-noir-hairline bg-pi-elevated px-3.5 py-3">
          <p className="text-[9px] text-pi-noir-text uppercase tracking-widest font-semibold mb-1.5">Category Creation Candidate</p>
      ...
      <div>
        <div className="flex items-baseline justify-between gap-3 mb-2">
          <p className="text-[10px] text-pi-noir-sub uppercase tracking-widest">Evidence Coverage</p>
          <span className={`font-mono text-lg font-bold ${cov.pct >= 50 ? 'text-pi-build-noir' : cov.pct >= 25 ? 'text-pi-gold-bright' : 'text-pi-risk-noir'}`}>{cov.pct}%</span>
        </div>
        <div className="h-1.5 rounded-full bg-pi-noir-hairline overflow-hidden mb-2">
          <div className={`h-full ${cov.pct >= 50 ? 'bg-pi-build-noir' : cov.pct >= 25 ? 'bg-pi-gold-deep' : 'bg-pi-risk-noir'}`} style={{ width: `${cov.pct}%` }} />
        </div>
      ...
      {/* Score Breakdown */}
      <div className="mt-7 pt-6 border-t border-pi-noir-hairline">
        <p className="text-[10px] text-pi-noir-sub uppercase tracking-widest mb-3">
            ...
            {scored.map(d => (
              <div key={d.key}>
                <div className="flex items-center gap-3">
                  <span className="text-xs text-pi-noir-sub w-40 shrink-0 truncate">{d.label}</span>
                  <div className="flex-1 h-1.5 rounded-full bg-pi-noir-hairline overflow-hidden">
                    <div className="h-full bg-pi-noir-text" style={{ width: `${(d.rawScore ?? 0) * 10}%` }} />
                  </div>
                  <span className="font-mono text-xs text-pi-noir-sub w-10 text-right shrink-0">{d.rawScore}/10</span>
        ...
        {qualitative.length > 0 && (
          <div className="mt-4 pt-3 border-t border-pi-noir-hairline space-y-2.5">
            <p className="text-[9px] text-pi-noir-sub uppercase tracking-wider">Not Scored — AI Judgment Only, 0% Weight</p>
            {qualitative.map(d => (
              <div key={d.key} className="flex items-center gap-3">
                <span className="text-xs text-pi-noir-sub w-40 shrink-0 truncate italic">{d.label}</span>
                <span className="flex-1 text-xs text-pi-noir-sub italic">{d.qualitativeLevel ?? 'Not assessed'}</span>
      ...
      <div className="mt-7 pt-6 border-t border-pi-noir-hairline">
        <div className="flex items-baseline justify-between gap-3 mb-2.5">
          <p className="text-[9px] text-pi-noir-sub uppercase tracking-wider">Evidence Breadth</p>
          <span className="font-mono text-xs text-pi-noir-sub">{evidenceBreadth.contributingProviders.length} / {evidenceBreadth.totalScoreEligibleProviders} providers</span>
        </div>
        <WitnessDots filled={evidenceBreadth.contributingProviders.length} total={evidenceBreadth.totalScoreEligibleProviders} variant="pi-noir" />
        ...
        <div className="mt-3 flex flex-wrap gap-1.5">
          {evidenceBreadth.channelBreakdown.map(c => (
            <span
              ...
              className={`text-[10px] rounded-full px-2 py-1 border ${c.contributed ? 'text-pi-build-noir bg-pi-elevated border-pi-noir-hairline' : 'text-pi-noir-sub bg-pi-elevated border-pi-noir-hairline'}`}
            >
              {c.label}
            </span>
          ))}
        ...
        <p className="mt-2.5 text-[10px] text-pi-noir-sub">
        ...
        {evidenceBreadth.contributingProviders.length > 0 && (
          <div className="mt-4 pt-4 border-t border-pi-noir-hairline">
            <p className="text-[9px] text-pi-noir-sub uppercase tracking-wider mb-2">Sources</p>
            <div className="flex flex-wrap gap-1.5">
              {evidenceBreadth.contributingProviders.map(p => (
                <span key={p} className="font-mono text-[10px] text-pi-noir-text bg-pi-elevated border border-pi-noir-hairline rounded-full px-2 py-1">{p}</span>
              ))}
            </div>
          </div>
        )}
        ...
        {contributedChannels.length > 0 && (
          <div className="mt-3 space-y-1">
            {contributedChannels.map(c => (
              <p key={c.channel} className="text-[10px] text-pi-noir-sub leading-relaxed">
                <span className="text-pi-noir-sub">{c.label}:</span> {CHANNEL_COVERAGE_NOTES[c.channel]}
              </p>
            ))}
          </div>
        )}
      ...
      {/* Consistency Check */}
      <div className="mt-7 pt-6 border-t border-pi-noir-hairline">
        <p className="text-[10px] text-pi-noir-sub uppercase tracking-widest mb-3">Consistency Check</p>
        {flags.length === 0 ? (
