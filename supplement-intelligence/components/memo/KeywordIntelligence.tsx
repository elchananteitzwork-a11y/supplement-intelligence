...
import { ProvenanceBadge, ProvenanceCaption, LabNoData, LabEmptyState, SectionIntro, PiCard } from './shared'
...
function ExpandableKeywordTable({ keywords, collapseAt = 5 }: { keywords: KeywordMetric[]; collapseAt?: number }) {
  ...
  return (
    <div>
      ...
      {hidden > 0 && !expanded && (
        <button onClick={() => setExpanded(true)} className="text-[11px] text-pi-gold-bright hover:underline transition-colors mt-2">Show {hidden} more →</button>
      )}
    </div>
  )
}
