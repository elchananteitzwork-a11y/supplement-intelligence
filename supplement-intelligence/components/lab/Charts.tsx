...
const PHOTON = '#4fa8ff'
const AMBER = '#f5b947'
const VERDANT = '#34d9a0'
const EMBER = '#ff6259'
const GRID = 'rgba(255,255,255,0.08)'
const AXIS_TEXT = '#686c78'
...
export function VolumeTrendChart({ history }: { history: { year: number; month: number; volume: number }[] }) {
  ...
  return (
    <div>
        ...
        {history.map((h, i) => {
          ...
          return (
            <rect
              ...
              rx={2} fill="rgba(79,168,255,0.16)"
            />
          )
        })}
      ...
      <div className="flex justify-between text-[9px] lab-text-data text-lab-text-tertiary mt-1.5">
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-auto">
      {bars.map((b, i) => {
        ...
        const x = PAD + i * barW
        const color = b.isPeak ? VERDANT : b.isLow ? EMBER : 'rgba(255,255,255,0.16)'
        return (
          <g key={b.name}>
...
export function OpportunityHeatmap({ metrics }: { metrics: KeywordMetric[] }) {
  const pts = [...metrics]
    ...
    .slice(0, 25)
  if (!pts.length) return <p className="text-xs text-lab-text-tertiary italic py-4 text-center">No competition-index data available for this query.</p>
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-auto">
      ...
      {pts.map((p, i) => {
        ...
        const color = score >= 60 ? VERDANT : score >= 35 ? AMBER : '#686c78'
    ...
    <div className="space-y-2">
      {withCounts.map(c => (
        <div key={c.label} className="flex items-center gap-3">
          <span className="text-xs text-lab-text-secondary w-28 shrink-0 truncate">{c.label}</span>
          <div className="flex-1 h-2 rounded-full bg-white/[0.06] overflow-hidden">
            <div className="h-full rounded-full bg-lab-photon/60" style={{ width: `${(c.count / maxCount) * 100}%` }} />
          </div>
          <span className="lab-text-data text-xs text-lab-text-secondary w-8 text-right shrink-0">{c.count}</span>
        </div>
      ))}
    </div>
  )
