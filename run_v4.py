"""
run_v4.py — V4 Product Discovery Engine CLI.
Think like a venture capitalist: find future brands, not just products.

Usage:
  python run_v4.py kitchen
  python run_v4.py pet
  python run_v4.py supplements
  python run_v4.py beauty
  python run_v4.py kitchen --max-reviews 300
  python run_v4.py --check-tokens
"""
...
from v4.engine import run as v4_run
from v4.output import write_csv, write_json, print_top

_OUTPUT_JSON = Path("v4-discovery-report.json")
_OUTPUT_CSV  = Path("v4-opportunities.csv")
...
def run_v4(
    ...
    print(f"\n{'#'*65}")
    print(f"  V4 Product Discovery Engine — Find Future Brands")
    ...
    if not products:
        ...
        print("  Try: --force-refresh, wider BSR range, higher --max-reviews.")
        return {}
    ...
    # ── Step 6: V4 scoring ────────────────────────────────────────────────────
    print("\n  Running V4 engine (7 components + anti-hype + opportunity gap)...")
    scores = v4_run(
        products=qualified,
...
def _print_summary(scores, elapsed, meta):
    ...
    gaps     = sum(1 for s in scores if s.opportunity_gap_bonus > 0)

    print(f"{'='*65}")
    print(f"  V4 DISCOVERY COMPLETE")
    ...
    print(f"  Gap bonus activated:     {gaps} product(s)  (0 expected with all stubs)")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="V4 Product Discovery Engine — find future brands, not just products.",
...
Examples:
  ...
  python run_v4.py supplements
  python run_v4.py beauty
  python run_v4.py kitchen --max-reviews 300 --max-subcategories 6
  python run_v4.py --check-tokens
        """,
    )
